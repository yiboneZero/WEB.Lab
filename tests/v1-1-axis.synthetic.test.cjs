...
const canvasContext=new Proxy({drawImage:noop,getImageData(){return{data:pixels}}},{get:(t,k)=>k in t?t[k]:noop,set:(t,k,v)=>(t[k]=v,true)});
element=new Proxy({width,height,hidden:false,disabled:false,value:'',textContent:'',innerHTML:'',style:{},classList:{add:noop,remove:noop,toggle:noop},addEventListener:noop,removeEventListener:noop,querySelector(){return element;},getContext(){return canvasContext},getBoundingClientRect(){return{left:0,top:0,width,height}}},{get:(t,k)=>k in t?t[k]:noop});
...
vm.runInContext(`${source}\nimage={};photoCanvas.width=${width};photoCanvas.height=${height};points=[{x:0,y:0},{x:40,y:0},{x:82,y:252},{x:618,y:102}];this.ok=refineOfficialAxisPoints();this.axis=shaftAxis;this.measured=distance(points[2],points[3]);this.constrained=constrainPointToShaftAxis({x:350,y:20});this.offAxis=Math.abs((this.constrained.x-shaftAxis.x)*shaftAxis.uy-(this.constrained.y-shaftAxis.y)*shaftAxis.ux);this.ballDetector=typeof findBallInRegion;points=[{x:0,y:0},{x:40,y:0}];this.autoOk=detectShaftAndEndpointsAutomatic();this.autoPoints=points.length;`,sandbox);
...
assert(sandbox.offAxis<.001,'manual endpoint adjustment must stay on the detected shaft axis');
